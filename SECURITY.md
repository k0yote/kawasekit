...
The full threat model — layer-by-layer surface, verdict per threat, and the
known limitations kawasekit chooses **not** to close in this release — lives
in [`docs/THREAT_MODEL.md`](docs/THREAT_MODEL.md). That document is the
authoritative reference for external security reviewers. If you find a
plausible attack that is not addressed there, please report it via the email
channel above rather than opening a public issue.
