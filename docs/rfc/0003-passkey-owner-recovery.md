...
| | |
|---|---|
| **Status** | **Cycle 1 (passkey owner) — DONE on Amoy 2026-06-18 (P1+P2, 20/20).** **Cycle 2 (recovery, Approach B) — ✅ DONE on Amoy 2026-06-20 (`recovery.test.ts` 6/6: C1-revalidate + R2 + R3/R4a + R4b + R4c; re-runnable; example PR merged).** The original A (bare passkey sudo + separate `WeightedECDSAValidator`) was un-buildable on Amoy (module not deployed; the new `@zerodev/weighted-validator` does 2-of-2 only as the SUDO) → **owner = a weighted validator [passkey 100, Hub 50, backup 50, threshold 100]; recovery = guardian-quorum config-reset to a new passkey** (gate txs in §6). Reopened Cycle 1's owner model (bare passkey → weighted sudo) and re-validated the session-key floor under the weighted sudo (C1-revalidate ✅). Headless `ox` signing reused via `toWebAuthnSigner`. **✅ Both Cycle-2 SDK gaps CLOSED in kawasekit `0.9.0`** (validator-agnostic `issueSessionKey({ sudoValidator, approveEnable })` = U-B1, `buildRevokeSessionKeyCall` = U-B2; G1 superseded) — verified e2e by the example consuming `0.9.0` (`recovery.test.ts` 6/6 on Amoy). See `docs/rfc/validator-agnostic-session-keys-{design,plan}.md`. |
...
### Cycle 2 — recovery (R3a, Approach B) — ✅ DONE on Amoy 2026-06-20 (recovery.test.ts 6/6 PASS)
Foundation gate (the prerequisite for this revision) PASSED on Amoy: deploy the weighted-sudo account [`0xcbfb7298…`], **guardians-only** config-reset to a new passkey [`0xd35fcf9a…`], new passkey signs at the same address [`0x87b2b0a7…`]. The full acceptance suite below was then formalized for Approach B and runs **green on Amoy** (`kawasekit-example/zerodev-passkey-jpyc/recovery.test.ts`, **6/6**: 2 offline unit + R2 + R3/R4a + R4b + R4c), **re-runnable** (R3/R4a ephemeral; R4b/R4c reset the owner back). The two offline unit tests pin the wire encodings: `doRecovery(address,bytes)` selector `0xac39fd0f` and `uninstallValidation(bytes21,bytes,bytes)` selector `0xe6f3d50a`.
- **C1-revalidate (the reopened Cycle-1 owner path) — ✅ PASS.** The session-key floor (RFC-0001 P2) holds under the **weighted sudo** [passkey 100, Hub 50, backup 50], not just a bare passkey sudo: `createBuyListPolicies` issued under the weighted-sudo owner → H1 + the §9 paymaster-less N1 pass. The agent floor survives the bare-passkey → weighted-sudo owner change. **SDK boundary U-B1:** the default single-signer enable that `serializePermissionAccount` emits is rejected by a weighted sudo (`EnableNotApproved` / `0xc48cf8ee`) — the fix is the passkey owner's `approvePlugin` + `encodeSignatures([approval], true)` passed as the 3rd arg (`serializePermissionAccount(account, undefined, enableSignature)`). *(Now the SDK's injected `approveEnable` seam — **closed in kawasekit 0.9.0**.)*
- **R1 (setup) — ✅ gate-proven** (deploy tx [`0xcbfb7298…`]). Account = sudo weighted [passkey 100, Hub 50, backup 50, threshold 100] + the recovery executor installed as a **fallback module** (`getRecoveryFallbackActionInstallModuleData` + `pluginMigrations`; NOT `plugins.action` → else `InvalidSelector`). Passkey alone signs normally (weight 100).
- **R2 (non-custodial negative — the core claim) — ✅ PASS.** A userOp signed by **the Hub alone** (weight 50 < 100) is **rejected on-chain** — and not merely for recovery: Hub alone is below threshold for **every** action. Run **paymaster-LESS** (self-paid POL) exactly as RFC-0001 §9: the negative threw "Signature provided for the User Operation is invalid" (the weighted-threshold `validateUserOp` revert / AA24) and was asserted **NOT** an `AA21`/prefund artifact — the airtight `validation_reject` boundary, not a `sponsor_reject` mask. The controlled pair holds: Hub-alone rejected / Hub+backup (100) succeeds. (Approach B has no separate recovery validator; the boundary is the weighted threshold itself.)
- **R3 (happy path) — ✅ PASS** (gate doRecovery tx [`0xd35fcf9a…`]; formalized in `recovery.test.ts`). Primary passkey "lost" → **Hub + user-backup (50+50=100)** sign `doRecovery` → the weighted config is reset to a **new passkey** (fresh `ox` credential) → the new passkey signs alone (weight 100); the **old passkey is gone** from the config.
- **R4 (post-recovery safety):**
  - **R4a — ✅ PASS** (gate same-address proof, new-owner tx [`0x87b2b0a7…`]). Recovery moves **no funds** (merchant/account `balanceOf` unchanged across the reset) and the account stays at the **same address** (Kernel v3.1: address = f(sudo + initial config); recovery changes the config, not the address).
  - **R4b — ✅ PASS.** The **new** passkey owner issues a **fresh** session key under the (reset) weighted sudo (via the U-B1 weighted enable) → the agent pays 0.001 JPYC on Amoy (merchant +amount).
  - **R4c (positive security) — ✅ PASS.** The new owner **revokes the old session key** → the same key can no longer pay (it fails at validation, merchant `balanceOf` unchanged in the post-revoke attempt); stale delegations persist across a root rotation in Kernel v3.1, so revoke is explicit. **SDK boundary U-B2:** kawasekit's `revokeSessionKey` calls `uninstallPlugin`, which hardcodes the single-signer `sendUserOperation` the weighted validator rejects → the harness reproduces the inner `uninstallValidation(vId, deinitData, "0x")` call (vId = `VALIDATOR_TYPE.PERMISSION ‖ pad(getIdentifier(),20)`, plugin rebuilt from the same session signer + same policies) and submits via the weighted `sendWeighted` aggregate flow. *(Now the SDK's `buildRevokeSessionKeyCall` — **closed in kawasekit 0.9.0**.)*

A passing run = **Cycle 1**: P1 + P2 (done; P2 re-validated under the weighted sudo = **C1-revalidate ✅**); **Cycle 2**: R1 / R3 / R4(a,b,c) succeed **and R2 (Hub-alone) is rejected on-chain** — all ✅ on Amoy 2026-06-20 (the gate independently proved the R1/R3/R4a mechanism; the merged example suite formalized C1-revalidate/R2/R4b/R4c). **SDK gaps — ✅ CLOSED in kawasekit `0.9.0`** (the Cycle-2 analogs of RFC-0001 G1, now shipped + verified e2e): **U-B1** — `issueSessionKey` / `createAgentSmartAccount` accept a pre-built `sudoValidator` (the `AgentOwner` union) + an injected `approveEnable` callback whose result threads into `serializePermissionAccount`'s 3rd arg, without kawasekit depending on `@zerodev/weighted-validator`. **U-B2** — `buildRevokeSessionKeyCall` returns the `uninstallValidation` callData for non-single-signer owners to submit via their aggregate flow (and `revokeSessionKey` is migrated onto the shared permission-validator helper so issue ↔ revoke derive the same vId). The example was re-pointed off its inline reproductions onto these SDK functions; **`recovery.test.ts` 6/6 on Amoy** (post-publish e2e of stable `0.9.0`). **G1 (bare-passkey issuance) is superseded** — the launch owner is the weighted sudo whose primary signer IS the passkey, so there is no separate bare-passkey path. See `docs/rfc/validator-agnostic-session-keys-{design,plan}.md`.
...
3. **Cycle 2 — recovery (R3a, Approach B) — ✅ DONE (Amoy 2026-06-20, `recovery.test.ts` 6/6; example PR merged).** On the **weighted-sudo owner**: recovery mechanism gate-proven (§8; deploy/reset/new-passkey txs), then the full suite formalized & green — **C1-revalidate** (session-key floor under the weighted sudo; U-B1 weighted enable), **R2** (Hub-alone rejected on-chain, **paymaster-less** — threw the weighted threshold / AA24, asserted not an AA21 prefund artifact), **R3/R4a** (config-reset, same-address, no funds moved), **R4b** (new owner re-provisions a session key → agent pays), **R4c** (new owner revokes the old key; U-B2 revoke via `sendWeighted`). `recovery.ts` rewritten onto `@zerodev/weighted-validator` (the wrong/older `@zerodev/weighted-ecdsa-validator` dropped). Re-runnable.
