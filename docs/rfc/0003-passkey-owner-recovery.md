...
| | |
|---|---|
| **Status** | Draft v2 — brainstorm-refined (2026-06-18): headless passkey signing via `webauthn-p256`+`signMessageCallback`; **2-slice** (Cycle 1 owner / Cycle 2 recovery); **R2 paymaster-less**; **R4→a/b/c** (re-issue fresh + revoke old = security-correct); passkey-server **deferred**. Pending owner review → writing-plans. |
...
| **SDK baseline** | kawasekit + ZeroDev Kernel v3.1 / EntryPoint 0.7; `@zerodev/webauthn-key` + passkey validator (duo-mode); `@zerodev/recovery` / WeightedECDSAValidator (ECDSA guardians); **`webauthn-p256`** (wevm — headless Node passkey signer for the harness). All new deps (none installed today). **Pin + verify symbols at implementation** (they drift). |
...
**Headless signing (how the harness produces passkey signatures — pure Node, no browser).** WebAuthn is normally browser-bound (`navigator.credentials`), but ZeroDev's `toWebAuthnKey({ webAuthnKey: { …parsedKey, signMessageCallback }, rpID })` exposes a generic **`signMessageCallback`** injection seam (the same one the react-native example uses for a native signer). The harness injects **`webauthn-p256`** (wevm, MIT) — a maintained, fully in-Node software authenticator: `createCredential()` makes a P256 credential, `sign({ credentialId, hash })` returns `{ signature, webauthn:{ authenticatorData, clientDataJSON, … } }`, i.e. exactly the `WebAuthnAuth(authenticatorData, clientDataJSON, r, s)` the on-chain verifier expects — no hand-rolled `authenticatorData`/`clientDataJSON`. So the harness stays **pure Node** (preserving RFC-0001's no-browser / independent-gate discipline). The one piece to pin (the **Cycle-1 foundation spike**, P1) is the thin **adapter** mapping `webauthn-p256.sign()` output → the exact return shape `signMessageCallback` expects.
...
- **SDK**: pin `@zerodev/webauthn-key` + the passkey validator, `@zerodev/recovery` / the weighted validator, and **`webauthn-p256`** (wevm — the headless Node signer); confirm exact symbols against the installed versions (none are installed today — all new deps; the ZeroDev passkey/recovery package names drift).
- **Foundation spikes (one per cycle, pin+verify at impl):** **C1** = the `webauthn-p256.sign()` → `signMessageCallback` adapter (format/byte match), proven by **P1**. **C2** = the exact ZeroDev recovery rotation mechanism (how the threshold validator authorizes the sudo swap on-chain), proven by **R1→R3**.
...
### Cycle 1 — passkey owner
- **P1 (foundation spike).** Account address derives from the passkey public key; a **passkey-signed userOp lands on Amoy** (P256 via RIP-7212 / Daimo fallback). The harness signs **headless** — `webauthn-p256.sign()` mapped to ZeroDev's `signMessageCallback` (§6.2). P1's DoD is the glue/format adapter — the **Cycle-1 foundation risk**; until P1 passes, do not layer recovery on top.
- **P2 (floor regression).** `createBuyListPolicies` issued **by the passkey owner** → re-run the full RFC-0001 acceptance: H1/H2 + sponsored N1–N4 (durable invariant) + the §9 **paymaster-less** N1–N4 (immutable on-chain boundary). All hold — the floor survives the ECDSA→passkey owner swap (no rotation; pure owner change).
...
- **R3 (happy path).** Primary passkey "lost" → **Hub + user backup co-sign (2-of-2)** → sudo rotated to a **new passkey** the user registers (a fresh `webauthn-p256` credential) → the new passkey transacts; the **old passkey is rejected**.
...
1. Approve this RFC.
2. **Cycle 1 — passkey owner** (`kawasekit-example` harness, consuming kawasekit). Foundation spike = the `webauthn-p256`→`signMessageCallback` adapter; land **P1** (passkey userOp on Amoy), then **P2** (RFC-0001 floor under the passkey owner, incl. the §9 paymaster-less negatives). Passkey server is **out of scope** (§6.1). Surface SDK boundary findings (the helpers a consumer needs to build a passkey account) the RFC-0001 way.
