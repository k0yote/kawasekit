...
| <a id="threat-5-5"></a>5.5 | Reasoning-step duplicate payment | 🟡 Known limitation | See §6.1. Mitigation is **agent-side**: pair tool calls with idempotency keys at the framework layer. kawasekit cannot enforce this from the SDK boundary because the SDK does not see the reasoning graph. |
